...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site',
        color='class',
        title='Success launches per sites')
        return fig
    else:
        ...
        names='class', 
        color='class',
        title='Launch success rate from {}'.format(entered_site))
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, payload_range):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        #data=pd.DataFrame(filtered_df.groupby(['Payload Mass (kg)','Booster Version Category'])['class'].mean().reset_index())
        ...
        # return the outcomes piechart for a selected site
        data = filtered_df[filtered_df['Launch Site']== entered_site]
        #data=pd.DataFrame(data.groupby(['Payload Mass (kg)','Booster Version Category'])['class'].mean().reset_index())
